...
# Global semaphore to throttle LLM/TTS concurrency across all sessions
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        if self.llm_inference_time is None:
            logger.warning("🗣️🧠🕒 LLM inference time unavailable (backend not ready); defaulting to 0 ms.")
    ...
    def _release_global_generation_lock_if_held(self):
        ...
        if self._has_global_generation_lock:
            try:
                self._global_generation_lock.release()
            except Exception:
                pass
            finally:
                self._has_global_generation_lock = False
    ...
    def _llm_inference_worker(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                # Acquire the global generation lock before starting heavy work
                if not self._has_global_generation_lock:
                    logger.info("🗣️🧠🔒 Waiting for global generation slot...")
                    self._global_generation_lock.acquire()
                    self._has_global_generation_lock = True
                    logger.info("🗣️🧠🔓 Acquired global generation slot.")
        ...
        *same* `audio_chunks` queue used by the quick worker. Handles stop requests
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                # Release global lock after final TTS finishes (success or abort)
                self._release_global_generation_lock_if_held()
    ...
    def process_abort_generation(self):
        ...
        with self.abort_lock:
            ...
            # Ensure the global generation lock is released on abort
            self._release_global_generation_lock_if_held()
    ...
    def shutdown(self):
        ...
        # As a final safeguard, release global lock if still held
        self._release_global_generation_lock_if_held()